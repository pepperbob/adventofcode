...
# credits to https://dev.to/qviper/advent-of-code-2020-python-solution-day-17-g6k

def findneighbour(cube):
    neighbors = [
        (cube[0]+a, cube[1]+b, cube[2]+c) 
        for a in range(-1, 2)
        for b in range(-1, 2)
        for c in range(-1, 2)
        if not (a,b,c)==(0,0,0)]
...
print(f"Solution 1: {list(pocketcube.values()).count('#')}")
